...
// ─── V3 — «Портрет» ──────────────────────────────────────────────────────────
// Portrait photo takes center stage. Clean asymmetric split.
// Desktop: left 42% text | right 58% photo. Mobile: photo first, text below.
export default function HeroV3({ isVisible }: HeroProps) {
  return (
    <section
    ...
    >
      ...
      >
        ...
        {/* ══════════════════════════════════════════════════
            PHOTO PANEL
            Mobile : order-1 (top), 62dvh
            Desktop: order-2 (right), full column height
        ══════════════════════════════════════════════════ */}
        <div
          className="relative order-1 md:order-2
            h-[62dvh] md:h-[calc(100dvh_-_var(--header-height))]
            flex items-stretch"
        >
        ...
        >
          ...
          <h1
            className="font-display font-light text-warm-dark leading-[1.03] tracking-[-0.03em] mb-5 md:mb-6"
          ...
          {/* Description */}
          <motion.p
            className="font-body text-warm-medium/80 text-sm md:text-base leading-relaxed mb-6 md:mb-7"
            style={{ maxWidth: '34ch' }}
          ...
          <motion.div
            className="grid grid-cols-3 divide-x divide-cream-400 border-t border-b border-cream-400 py-4 mb-6 md:mb-8"
            initial={{ opacity: 0 }}
            animate={isVisible ? { opacity: 1 } : { opacity: 0 }}
            transition={{ duration: 0.6, delay: 0.86 }}
          >
            {counters.map((c, i) => (
              <div key={i} className={`flex flex-col gap-1.5 ${i > 0 ? 'pl-3 md:pl-5' : ''}`}>
                <span
